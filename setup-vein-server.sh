...
# This script installs and configures a VEIN game server with a TUI interface
...
install_vein_server() {
    ...
    echo -e "   ${YELLOW}⚙️  Installing VEIN server (this may take a while)...${NC}"
    
    if runuser -l steam -c "~/Steam/steamcmd.sh +login anonymous +force_install_dir ${INSTALL_PATH} +app_update 2131400 validate +quit" > /dev/null 2>&1; then
...
create_systemd_service() {
    ...
    if [ "${AUTO_UPDATE}" = "true" ]; then
        AUTO_UPDATE_CMD="ExecStartPre=/home/steam/Steam/steamcmd.sh +login anonymous +force_install_dir ${INSTALL_PATH} +app_update 2131400 validate +quit"
    fi
...
${AUTO_UPDATE_CMD}
ExecStart=${INSTALL_PATH}/VeinServer.sh
